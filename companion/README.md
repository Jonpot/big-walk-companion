...
`node launch.mjs` starts or reuses a matching background server and opens the browser; `node launch.mjs --stop` stops it. The packaged installer registers this launcher with the mod for automatic game startup. Launcher and server output are written into the selected notes directory. Set `COMPANION_OPEN_BROWSER=0` for unattended launcher testing.
