...
test('starting a new package replaces an older matching server and preserves its saved notes',async()=>{
  ...
  try{const result=await run(env);assert.equal(result.code,0,result.output);const health=await(await fetch(origin+'/api/health')).json();pid=health.pid;assert.equal(stopped,1);assert.equal(health.appVersion,'0.3.0');const state=await(await fetch(origin+'/api/state')).json();assert.equal(state.revision,7);assert.equal(state.pack.version,2);assert.deepEqual(state.pack.areas,saved.pack.areas);assert.deepEqual(JSON.parse(await fs.readFile(path.join(fixture,'route-pack.json'),'utf8')),saved);}
