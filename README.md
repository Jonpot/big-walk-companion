...
Download the ZIP from [Releases](https://github.com/Jonpot/big-walk-companion/releases/latest), extract it, close Big Walk, and double-click **Install.cmd**. It locates the Steam installation, installs BepInEx and the companion plugin, and opens the game and map app. Node.js and the loader are included; no SDK is needed.

Keep the companion's command window open. Join a world and allow several minutes for the first modded launch. The plugin exports the map from your installed game; if the map stays blank, open the in-game paper map. Automatic texture export is new in this release and still needs first-run validation on another machine. Game artwork is not included in this repository or download.
