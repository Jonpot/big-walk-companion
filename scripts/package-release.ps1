param([string]$NodePath = '', [switch]$SkipBuild, [string]$Version='0.1.0')
...
foreach ($name in @('Install.cmd', 'Start Companion.cmd', 'START HERE.txt')) {
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot "distribution\$name") -Destination $stage
}
foreach ($name in @('GamePath.ps1', 'Install.ps1', 'Start.ps1')) {
